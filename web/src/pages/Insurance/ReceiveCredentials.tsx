...
const ReceiveCredentials: React.FC = () => {
    ...
    useEffect(() => {
        dispatch?.({
            type: Actions.REQUEST_ISSUANCE, provider: Providers.Impierce, scope: Scopes.Insurance, credentials: ["CompanyCredential"],
            issuer: Issuers.Insurance
        })
    }, [dispatch]);

    useEffect(() => {
        if(state[Scopes.Insurance]?.issuanceComplete) {
            goToNextStep();
        }
    }, [state, goToNextStep])
            ...
            <RandomGraphicElement elements={5}>
                <div className='scan-qr-page-wrapper'>
                    ...
                    <div className='qr-wrapper'>
                        ...
                        <QRCode text={state[Issuers.Insurance]?.QRcontent ?? ""} />
                    </div>
