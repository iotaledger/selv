        ...
        const _createAndStore = (schemaName, identity, data) => {
            return createCredential(identity, schemaName, data).then((credential) => {
                return storeCredential(schemaName, credential).then(() =>
                    console.log(`Credential with schema name ${schemaName} successfully created!`)
                );
            });
        };
        ...
        retrieveIdentity('did').then((identity) => {
            requestedCredentials
                ...
                .then((verifiablePresentations) => {
                    ...
                    const payload = encrypt(password, JSON.stringify(verifiablePresentations));
...
<div>
    ...
    <button on:click="{processVerifiablePresentations}">Create Verifiable Presentation</button>
    {#if displayScanner}
